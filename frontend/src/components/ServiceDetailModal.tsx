      ...
      <div className="card max-h-[90vh] w-full max-w-2xl overflow-y-auto">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          <div className="rounded-lg bg-[var(--bg)] p-3">
            <p className="text-xs text-[var(--muted)]">{t('cliente.status')}</p>
            <p className="font-medium capitalize">{t('kanban.' + servicio.estado)}</p>
          </div>
          <div className="rounded-lg bg-[var(--bg)] p-3">
            <p className="text-xs text-[var(--muted)]">{t('service.cost')}</p>
            <p className="font-medium text-emerald-400">${servicio.costoEstimado.toLocaleString()}</p>
          </div>
          <div className="rounded-lg bg-[var(--bg)] p-3">
            <p className="text-xs text-[var(--muted)]">{t('service.client')}</p>
            <p className="font-medium">{servicio.clienteId?.nombre}</p>
          </div>
          <div className="rounded-lg bg-[var(--bg)] p-3">
            <p className="text-xs text-[var(--muted)]">{t('service.technician')}</p>
        ...
        <h3 className="mb-3 flex items-center gap-2 font-semibold">
          ...
          {t('service.observations')}
        </h3>

        <div className="space-y-4">
          {obsByPhase.map((phase) => (
            <div key={phase.id} className="rounded-lg border border-[var(--border)] p-3">
              <p className="mb-2 text-sm font-medium">{t('kanban.' + phase.id)}</p>
              {phase.items.length === 0 ? (
                <p className="text-xs text-[var(--muted)]">{t('service.noObservations')}</p>
              ) : (
                  ...
                  {phase.items.map((o, i) => (
                    <li key={i} className="rounded bg-[var(--bg)] p-2 text-sm">
                      ...
                      <p className="mt-1 text-xs text-[var(--muted)]">
                        {typeof o.autorId === 'object' ? o.autorId.nombre : t('roles.cliente')} ·{' '}
                        {new Date(o.fecha).toLocaleString(lang === 'es' ? 'es-CO' : lang)}
                      </p>
                    </li>
                  ))}
                </ul>
        ...
        <div className="mt-4 border-t border-[var(--border)] pt-4">
            <p className="label">{t('service.addObservation')}</p>
            <select
            ...
            <textarea
              ...
              placeholder={t('service.observationPlaceholder')}
              value={texto}
              onChange={(e) => setTexto(e.target.value)}
            />
            {error && <p className="mb-2 text-sm text-red-400">{error}</p>}
            <button onClick={addObservacion} disabled={loading} className="btn-primary">
              {loading ? t('common.loading') : t('service.addObservation')}
            </button>
          </div>
      </div>
    </div>
